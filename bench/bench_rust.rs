...
pub fn encode(text: &str, secret_key: &str) -> String {
    ...
    let key_len = key.len();
    if key_len == 0 {
        // ключ пустой -> весь текст в base64 (без маркеров/реверса, упрощение)
        let mut reversed = reverse_bytes(src);
        reversed.extend_from_slice(MARKER.repeat(0).as_slice());
...
pub fn decode(encoded: &str, secret_key: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    res = BASE64.decode(encoded.trim().as_bytes())?;
...
fn main() {
    ...
    // Для больших decode: заранее кодируем вход, чтобы decode получил честный base64 вход
    ...
    println!(
        "Rust {} | input={}B | iters={} | total={:.3}ms | avg={:.3}us/iter | {:.2} MB/s | result_len={}",
